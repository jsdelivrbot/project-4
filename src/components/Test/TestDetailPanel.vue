<template>
  <transition name="edit" appear>
      <div class="editPanel" id="testDetailPanel">
        <div class="edit-panel">
          ...
          <div class="details-panel-fields">
            ...
            <p class="paragraph" :title="TEST.TestTask.title"
               v-show="showParagraph"
            >
              <input style="border:none;width:100%;" v-model="TEST.TestTask.title" @keyup="switchInput">
            </p>
            <!--edit comments-->
            ...
            <div class="po-field-wrapper edit-list">
              ...
              <!--<div class="editPanelPlanlet">
                <i class="icon-planlet">
                  <span id="taskid_panel">  ID{{TestTask.id}}</span>
                </i>
              </div>-->
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      ...mapState(['newTaskId','TEST','flagForNewTitle'])
    },
    methods: {
      assignedName(){
      ...
      switchInput: function(e){//编辑文本和图片内容
        var id = $('#taskid_panel')[0].innerText.substr(3);
        //$('#toDo div[taskid=' + id + '] h4')[0].innerText = $(e.target).val();
      },
      ...
      toolBtnSave: function() {
        ...
        var param = {
          token: user_token,
          taskId: _this.currentCardInfo.taskId,
          projectId: 181,
          data: [
              {id:101,value:_this.currentCardInfo.values.title},
              {id:108,value:_this.currentCardInfo.values.assigned},
              {id:601,choiceid:_this.currentCardInfo.values.status}
            ]
        }
        this.$store.dispatch('saveEditTask',param);
      ...
      closeEditPanel: function(){
        this.changeEditPanelStatus(false);
        if($('.groupbycontent').length > 0){
          $('.groupbycontent').width(3210);
          $('.statusbar').scrollLeft(1920);
        }
      },
      ...
      ...mapMutations(['changeEditPanelStatus','changeNewTaskTitle','TEST'])
    }
  }
</script>
<style lang="scss" scoped="" type="text/css">
  @import './TestDetailPanel.scss';
</style>
