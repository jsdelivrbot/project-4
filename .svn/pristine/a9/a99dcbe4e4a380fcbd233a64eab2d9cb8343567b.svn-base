<template>
    <div class="manageFolder">
        ...
        <ul v-show="open" v-if="isFolder" class="node">
            <manage-tree v-for="(item,$index) in model.nodes"  :key="$index" :model="item"></manage-tree>
        </ul>
    </div>
</template>
<script> 
    ...
    computed: {
      ...
      ...mapState(['MANAGE','linkedSpaceName'])
    },
    methods: {
      toggle(item) {
        if (this.isFolder) {
      ...
      projectClick(item){
        this.changeManageInfo({projectId:item.projectId,subProjectId:item.subprojectId})
        this.getManagementTasks();
        this.changeManageSearchPanel(false);
        this.changeBoardsBackend(false)
      },
      ...mapMutations(['changeBoardsBackend','changeManageSearchPanel','changeManageInfo','changeCurPath','changeBoardsBackend','changeIds']),
      ...mapActions(['getManagementTasks'])
    }
  }
</script>
