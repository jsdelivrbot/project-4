                ...
                <div class="attach-upload">
                  ...
                  <template>
                    ...
                    <Upload
                        ...
                        style="width:104px;float:left;position:relative; border:none !important;">
                        <div style="width: 104px;height:124px; line-height:126px; border:none !important;">
                            <Icon type="arrow-up-a" size="40">Add Attachment</Icon>
                        </div>
                    </Upload>
...
<script>
  ...
  export default {
    ...
    watch:{
      currentCardInfo:{//问题：刷新页面的时候再次传过来的currentCardINfo和之前的相同，但是已经有的附件图片因为不会出发请求函数而不见了
            ...
            if(res.status == 200){
              ...
              if(attachInfoList && attachInfoList.length > 0){
                  for(var i=0;i<attachInfoList.length;i++){
                     ...
                     // console.log(i+":" + 'FileID='+ attachInfoList[i].FileID);
                     // console.log(i+":" + 'attachInfoArr = '+ JSON.stringify(attachInfoArr));
                  }
                  _this.$refs.upload.fileList = attachInfoArr;
                  _this.uploadList = _this.$refs.upload.fileList;
                  sessionStorage.setItem("z-uploadFileList",JSON.stringify(_this.$refs.upload.fileList));
              }
            }
          },err=>{
            console.log(err);
