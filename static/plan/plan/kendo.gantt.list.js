    ...
    var GanttList = ui.GanttList = Widget.extend({
        ...
        _trs: function(tasks) {
            ...
            for (var i = 0, length = tasks.length; i < length; i++) {
                ...
                // [Change -- add default class]
                className.push("dp-gantt-tree-row")
        ...
        _td: function(options) {
            ...
            if (column.field === "title") {
                ...
                if (task.icon) {
                    children.push(kendoDomElement("span", {
                        className: listStyles.icon + " " + task.icon
                    }));
                }
        ...
        _editable: function() {
            ...
            if (!mobileOS) {
                that.content
                    ...
                    //.on("dblclick" + NS, "td", function (e) { // [Change -- double click to single click]
                    .on("click" + NS, "td", function(e) {
                        //if (!$(e.target).is(iconSelector)) { // [Change -- click on text]
                        if ($(e.target).is("span") && !$(e.target).is(iconSelector)) {
        ...
        _editCell: function(options) {
            ...
            // [Change -- add Check Function]
            if (typeof doGanttListInlineEditCheck === 'function') {
                if (!doGanttListInlineEditCheck(model)) {
                    return;
                }
            }
        ...
        _draggable: function() {
            ...
            var dropAllowedAsChild = true; // [Change -- add]
            var dropAllowedAsSibling = true; // [Change -- add]
            var dropTarget;
            var listStyles = GanttList.styles;
            ...
            var allowDrop = function(task) {
                ...
                //[Change -- add check function]
                if (dropAllowed) {
                    if (typeof doGanttListDragDropCheck == 'function') {
                        dropAllowedAsChild = doGanttListDragDropCheck(draggedTask, task);
                        parent = that.dataSource.taskParent(task);
                        dropAllowedAsSibling = doGanttListDragDropCheck(draggedTask, parent);

                        dropAllowed = dropAllowedAsChild || dropAllowedAsSibling;
                    }
                }
            };
            var defineLimits = function() {
                ...
                if (dropAllowedAsChild) {
                    if (dropAllowedAsSibling) {
                        extend(dropTarget, {
                            beforeLimit: offsetTop + height * 0.25,
                            afterLimit: offsetTop + height * 0.75
                        });
                        ...
                        extend(dropTarget, {
                            beforeLimit: offsetTop,
                            afterLimit: offsetTop + height
                        });
                    }
                }
                else {
                    if (dropAllowedAsSibling) {
                        extend(dropTarget, {
                            beforeLimit: offsetTop + height * 0.5,
                            afterLimit: offsetTop + height * 0.5
                        });
                    }
                }
            };
            ...
            this._tableDropArea = this.content
                .kendoDropTargetArea({
                    ...
                    "drop": function() {
                        ...
                        // [Change -- doGanttListDragDropMove is define go this function to handle drop, otherwise go default function]
                        if (typeof doGanttListDragDropMove == 'function') {
                            if (dropAllowed) {
                                doGanttListDragDropMove(draggedTask, target, action);
                            }
                        }
                        else {
