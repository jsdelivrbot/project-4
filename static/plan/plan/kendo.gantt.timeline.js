...
(function($) {
    ...
    //[Change -- add view 1,2,4,8,16,32,64]
    var defaultViews = {
        view1: {
            type: "kendo.ui.GanttView1"
        },
        view2: {
            type: "kendo.ui.GanttView2"
        },
        view4: {
            type: "kendo.ui.GanttView4"
        },
        view8: {
            type: "kendo.ui.GanttView8"
        },
        view16: {
            type: "kendo.ui.GanttView16"
        },
        view32: {
            type: "kendo.ui.GanttView32"
        },
        view64: {
            type: "kendo.ui.GanttView64"
        },
        day: {
    ...
    var GanttView = kendo.ui.GanttView = Widget.extend({
        ...
        _tasksTable: function(tasks) {
            ...
            for (var i = 0, l = tasks.length; i < l; i++) {
                ...
                // [Change -- add default class and uid]
                row = kendoDomElement("tr", { className: "dp-gantt-view-row", "data-uid": task.uid });
        ...
        _renderTask: function(task, position) {
            ...
            //[Change -- add control folder && task story item && new way to define summary && cannot be show tasks]
            if (!task.canBeShown()) {
                taskElement = this._renderDateNullTask(task, position);
            } else if (task.typeId === 97 || task.typeId === 98 || task.typeId === 99 || task.typeId === 100) {
                taskElement = this._renderControlFolder(task, position);
            } else if (task.typeId === -1 || task.typeId === -2 || task.typeId === -3 || task.typeId === -4) {
                taskElement = this._renderSingleTask(task, position);
            } else if (task.summary && !task.IsLeafFolder) {
...
//            if (!(task.summary && !task.IsLeafFolder) && !task.isMilestone() && editable && this._taskTemplate === null) {
...
//            }
        ...
        //[Change -- add date-is-null task]
        _renderDateNullTask: function (task, position) {
            var styles = GanttView.styles;
            var element = kendoDomElement("div", { className: styles.task + " k-task-datanull " + task.barStyle, "data-uid": task.uid });

            return element;
        },

        //[Change -- add control folder]
        _renderControlFolder: function (task, position) {
            var styles = GanttView.styles;
            ...
            var element = kendoDomElement("div", { className: styles.task + " " + styles.taskSummary + " " + task.barStyle, "data-uid": task.uid, style: { width: Math.max((position.width - position.borderWidth * 2), 0) + "px"} }, [
                    content
                ]);
        ...
        _renderSingleTask: function(task, position) {
            ...
            // [Change -- Always show progress bar]
            ...
            if (this.options.editable) {
                //[Change -- remove 'Delete' action and icon]
            ...
            //[Change -- add task.barStyle]
            var element = kendoDomElement("div", {
                className: styles.task + " " + styles.taskSingle + " " + task.barStyle, "data-uid": task.uid, style:
        ...
        _renderMilestone: function(task) {
            var styles = GanttView.styles;
            //[Change -- add task.barStyle]
            var element = kendoDomElement("div", { className: styles.task + " " + styles.taskMilestone + " " + task.barStyle, "data-uid": task.uid });
        ...
        _renderSummary: function(task, position) {
            ...
            var progressWidth = Math.round(position.width * task.percentComplete);
            //[Change -- add task.barStyle]
            var element = kendoDomElement("div", { className: styles.task + " " + styles.taskSummary + " " + task.barStyle, "data-uid": task.uid, style: { width: position.width + "px"} }, [
        ...
        _createDragHint: function(element) {
            this._dragHint = element
                ...
                .css("cursor", "move")
                .css("z-index", "10"); //[Change -- add z-index]

            //[Change -- 1. remove useless elements, 2. set opacity and border]
            this._dragHint.find(".k-task-dot").remove();
            this._dragHint.find(".k-task-draghandle").remove();
            this._dragHint.find(".k-task").children().remove();
            this._dragHint.find(".k-task").css({ "opacity": "0.5", "border": "2px dashed black" });
        ...
        */
        // [Change -- new way to show resize drag-drop]
        _createResizeHint: function (element, task) {
            this._resizeHint = element
                .clone()
                .addClass(GanttView.styles.dragHint)
                .css({ "cursor": "e-resize", "z-index": "10", "left": 0, "width": this._contentWidth });

            this._resizeHint.find(".k-task-dot").remove();
            this._resizeHint.find(".k-task-draghandle").remove();
            this._resizeHint.find(".k-task").children().remove();
            this._resizeHint.find(".k-task").css({ "opacity": "0.5", "border": "2px dashed black" });

            element
                .parent()
                .append(this._resizeHint);
        ...
        _updateResizeHint: function(start, end, resizeStart) {
            ...
            this._resizeHint.find(".k-task") // [Change -- adjust .k-task]
                .css({
                    "left": left,
                    "width": width
                });
        ...
        _renderCurrentTime: function() {
            ...
            element.css({
                ...
                top: "0px",
                width: "1px",
                height: "0px" //this._contentHeight + "px" [Change -- make height 0px]
            })
            .appendTo(this.content);
        },
    ...
    var GanttTimeline = kendo.ui.GanttTimeline = Widget.extend({
        ...
        _range: function(tasks) {
            ...
            var start = new Query(tasks).filter(startFilter).sort(startOrder).toArray()[0].start || new Date();
            ...
            var end = new Query(tasks).filter(endFilter).sort(endOrder).toArray()[0].end || new Date();
        ...
        _resizable: function() {
            ...
            this._resizeDraggable
                .bind("dragstart", function(e) {
                    ...
                    // [Change -- Call new function]
                    that.view()._createResizeHint(element.parent(), task);
        ...
        _selectable: function() {
            ...
            if (this.options.selectable) {
                this.wrapper
                    ...
                    .on(DBLCLICK + NS, DOT + styles.line, function (e) {
                        // [Change -- Add double click event for edit dependency]
                        e.stopPropagation();
                        var uid = $(this).attr("data-uid");
                        if (typeof doEditDependency == 'function') {
                            doEditDependency(uid);
                        }
                    })
                ...
                if (!kendo.support.mobileOS) {
                    this.wrapper
                        .on(DBLCLICK + NS, DOT + styles.task, function(e) {
                            //[Change -- go our editing function]
                            if (typeof doEidtTask == 'function') {
                                var uid = $(this).attr("data-uid");
                                doEidtTask(uid);
                            }
                            else {
